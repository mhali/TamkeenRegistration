...
{
    public partial class PastQuizzes : System.Web.UI.Page
    {
        ...
        protected void ViewQuizAnswers_OnClick(object sender, EventArgs e)
        {
            int QuizId = Convert.ToInt32((sender as LinkButton).CommandArgument);

            //Response.Write("<script>");
            //Response.Write("window.open('QuizAnswers.aspx?QuizId=" + QuizId.ToString() + "&target=_blank')");
            //Response.Write("</script>");


            Response.Write("<script>");
            Response.Write("window.location.href='QuizAnswers.aspx?QuizId=" + QuizId.ToString()+"'");
            Response.Write("</script>");

            //Response.Write("<script>");
            //Response.Write("window.location.replace('QuizAnswers.aspx?QuizId=" + QuizId.ToString()+"')");
            //Response.Write("</script>");

            //Response.Write("<script>");
            //Response.Write("window.location.href='QuizAnswers.aspx?QuizId=" + QuizId.ToString()); //"window.open('TamkeenerAttendance.aspx?TamkeenerId=" + Tamkeener_ID.ToString() + "&target=_blank')"
            //Response.Write("</script>");
        ...
        protected void btnVideo_Save_Click(object sender, EventArgs e)
        {
            ...
            sqlCmd.Parameters.AddWithValue("@VideoUrl", txtVideoUrl.Text.Trim().Replace("watch?v=", "embed/"));
        ...
        protected void ViewQuiz_OnClick(object sender, EventArgs e)
        {
            int QuizID = Convert.ToInt32((sender as LinkButton).CommandArgument);
            LoadQuizById(QuizID);
        }
        ...
        private void LoadQuizById(int QuizId)
        {
            ...
            if (dtbl.Rows[0]["VideoUrl"].ToString() != "")
            {
                QuizVideo.Src = txtVideoUrl.Text;
            }
            else
            {
                QuizVideo.Src = "";
